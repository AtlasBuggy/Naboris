...
from naboris.depth import model


class DepthPipeline(Pipeline):
    def __init__(self, course_model_dir, refine_model_dir, enabled=True, log_level=None):
        super(DepthPipeline, self).__init__(enabled, log_level)
        ...
        coarse = model.inference(images, self.keep_conv, trainable=False)

        # instantiate fine model placeholder. The output (logits) is the depth image
        self.logits = model.inference_refine(images, coarse, self.keep_conv, self.keep_hidden)
